...
let promise3=new Promise((resolve,reject)=>resolve(1));

/* 3.Promise.prototype.then() */
// then方法返回的是一个新的Promise实例（注意，不是原来那个Promise实例）。
// 因此可以采用链式写法，即then方法后面再调用另一个then方法
// 第一个回调函数完成以后，会将返回结果作为参数，传入第二个回调函数
getJSON('/post.json').then(json=>json.post).then(post=>true);
...
const promiseAll=[2, 3, 5, 7, 11, 13].map(function(id){
	return getJSON('/post/' + id + ".json");
});
Promise.all(promiseAll).then(function(posts){
...
.then(result => result)
.catch(e => e);  // 如果没有catch就会调用all中的catch方法
...
const p3 = Promise.race([p1, p2, p3]);
